...
export const ChatMessageItem: React.FC<ChatMessageItemProps> = ({
  ...
  const isGuidingQuestion =
    ...
    message.content.toLowerCase().includes('think about') ||
    ...
    >
      ...
      <div
        className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center shrink-0 shadow-xs ${
          isAssistant
            ? 'bg-linear-to-br from-indigo-600 to-violet-600 text-white shadow-indigo-500/20'
      ...
      <div className="flex-1 min-w-0 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
            {isAssistant && onSpeak && (
              ...
              >
                {isSpeaking ? <VolumeX className="w-3.5 h-3.5 text-indigo-700 animate-pulse" /> : <Volume2 className="w-3.5 h-3.5" />}
              </button>
            )}
            <button
              type="button"
        ...
        {isAssistant && isGuidingQuestion && (
          <div className="bg-amber-50 border border-amber-200 px-3 py-2 rounded-xl text-xs font-semibold text-amber-900 flex items-center gap-2">
            ...
            <span>
              <strong>Guiding Question:</strong> Reflect on this step and reply with your intuition or calculations!
            </span>
          </div>
        )}
